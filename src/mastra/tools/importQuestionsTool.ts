...
const processImportFile = async (file: { filename: string, content: string, contentType: string }) => {
  try {
    ...
    if (questions.length > 0) {
      const now = new Date();
      const preparedQuestions: Omit<Question, '_id'>[] = questions.map(q => ({
...
const processJsonFile = async (content: string): Promise<QuestionData[]> => {
  try {
...
const processCsvFile = async (content: string): Promise<{questions: QuestionData[], failedCount: number}> => {
  try {
    const result = parse<CsvRow>(content, {
      header: true,
      skipEmptyLines: true,
      dynamicTyping: true,
    });
    ...
    for (const row of result.data) {
      try {
        ...
        validQuestions.push({
          ...
          difficulty: difficulty as 'junior' | 'mid-level' | 'senior',
...
const validateQuestions = (questions: unknown[]): QuestionData[] => {
  const validQuestions: QuestionData[] = [];
  ...
  for (const q of questions) {
    try {
      ...
      if (typeof question.technologies === 'string') {
        technologies = question.technologies.split(',').map((tech: string) => tech.trim());
