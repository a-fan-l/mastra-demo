import { createClient, Value, ResultSet, Client } from '@libsql/client';
import { z } from 'zod';
...
interface InteractionHistory {
  userId: string;
  questionId: string;
  interaction: 'viewed' | 'answered' | 'skipped' | 'liked' | 'disliked';
  timestamp: Date;
}
...
function valueToDate(value: Value): Date {
  return value === null ? new Date() : new Date(String(value));
}

// Helper function to parse JSON safely
function safeJSONParse<T>(value: Value, defaultValue: T): T {
...
export class Database {
  ...
  async importQuestions(questions: Omit<Question, 'id' | 'dateAdded'>[]): Promise<{ success: boolean, count: number, error?: any }> {
    try {
  ...
  async recordInteraction(userId: string, questionId: string, interaction: 'viewed' | 'answered' | 'skipped' | 'liked' | 'disliked') {
    try {
      await this.client.execute({
  ...
  async getTrainingData(limit = 100): Promise<TrainingData[]> {
    try {
      const { rows } = await this.client.execute({
      ...
      return rows.map(row => ({
        ...
        difficulty: valueToString(row.difficulty) as 'junior' | 'mid-level' | 'senior',
  ...
  async getQuestions(difficulty: string, technologies: string[], limit: number): Promise<Question[]> {
    try {
      const { rows } = await this.client.execute({
        sql: `
          ...
          LIMIT ?
        `,
        args: [difficulty, `%${technologies.join('%')}%`, limit]
      });
      return rows.map(row => ({
  ...
  async updateQuestionStats(questionIds: string[]) {
    try {
